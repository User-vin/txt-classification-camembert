...
def get_inputs_and_labels(df: pd.DataFrame):
    ...
    date_labels = tf.convert_to_tensor(df["date_interval"].tolist(), dtype=tf.float32)
...
def main():
    ...
    test_df.to_pickle(config.TRAIN_PKL_PATH)
    ...
    train_information = information(df=train_df, folder_path=config.FIGURES_PATH, prefix="train_segment", y_axis_label="Nombre de segments")
    test_information = information(df=test_df, folder_path=config.FIGURES_PATH, prefix="test_segment", y_axis_label="Nombre de segments")
    val_information = information(df=val_df, folder_path=config.FIGURES_PATH, prefix="val_segment",y_axis_label="Nombre de segments")
    ...
    train_df = split_dataframe(df=train_df, size=0.1, random_state=40)
    test_df = split_dataframe(df=test_df, size=1.0, random_state=40)
    val_df = split_dataframe(df=val_df, size=0.1, random_state=40)
